...
A platform multi-projekt felépítésű. Az `APP_PROJECT` környezeti változó határozza meg az aktív projektet (alapértelmezés: `personal_assistant`). A backend a `projects/<project>/` könyvtárból tölti be az ágenseket, root ágenseket és flow-kat.

```
projects/
  personal_assistant/               # ── Egy projekt ──
    agents/
      ...
      user_agent/                   # Felhasználói interakciós ágens
        ...
        prompts/system_prompt.md
      tools/
        mcp_server.py               # Megosztott MCP szerver
    ...
    flows/
      ...
      simple_code_task.flow.yaml    # Kód generálási flow

config/                             # ── Megosztott konfiguráció ──
  llm_providers.yaml                # LLM provider/modell/árazás
```

**Projekt váltása:**
- `.env`: `APP_PROJECT=personal_assistant`
- Docker: `docker-compose.yaml` automatikusan a `${APP_PROJECT}` alapján mount-olja a megfelelő projekt könyvtárat
